...
def normalize_taint_kind(value: object, *, default: TaintKind) -> TaintKind:
    ...
    return _TAINT_KIND_ALIASES.get(key, default)
...
def project_taint_ledgers(
    *,
    ...
    for raw_entry in marker_rows:
        ...
        taint_kind = normalize_taint_kind(
...
def _build_witness(
    *,
    ...
    if not policy_basis or not justification_code:
        return None
    eraser_id = str(
...
def _status_for_entry(
    *,
    ...
    if boundary_payload.allowed_taint_kinds and taint_kind not in boundary_payload.allowed_taint_kinds:
        return TaintStatus.ILLEGAL_LOCUS
