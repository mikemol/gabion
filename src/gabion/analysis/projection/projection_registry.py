...
from gabion.analysis.projection.projection_normalize import (
    normalize_spec, spec_canonical_json, spec_hash)
...
from gabion.analysis.foundation.timeout_context import (
    Deadline, deadline_clock_scope, deadline_scope, forest_scope)
...
PROJECTION_FIBER_REFLECTION_SPEC = ProjectionSpec(
    spec_version=1,
...
PROJECTION_FIBER_CONTEXT_WEDGE_SPEC = ProjectionSpec(
    spec_version=1,
...
PROJECTION_FIBER_REINDEX_SPEC = ProjectionSpec(
    spec_version=1,
...
PROJECTION_FIBER_NEGATED_EXISTENTIAL_IMAGE_SPEC = ProjectionSpec(
    spec_version=1,
...
def spec_metadata_lines_from_payload(payload: Mapping[str, JSONValue]) -> list[str]:
    ...
    spec_payload = mapping_optional(payload.get("generated_by_spec")) or {}
