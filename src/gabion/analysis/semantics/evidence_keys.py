...
def _normalize_suite_site_endpoint(payload: object, *, role: str) -> dict[str, object]:
    endpoint_mapping = mapping_optional(payload)
    ...
    key_payload = sequence_optional(endpoint_mapping.get("key"))
...
def _normalize_suite_site_endpoints(payload: object) -> dict[str, object]:
    endpoints_mapping = mapping_optional(payload)
...
class CanonicalAspfPathPayload:
    ...
    def from_mapping(cls, payload: Mapping[str, object]) -> "CanonicalAspfPathPayload":
        ...
        basis_payload = sequence_optional(payload.get("basis_path"))
    ...
    def validate(self) -> None:
        ...
        if self.suite_site_endpoints:
            ...
            source_key = sequence_optional(normalized_endpoints["source"].get("key"))
            target_key = sequence_optional(normalized_endpoints["target"].get("key"))
