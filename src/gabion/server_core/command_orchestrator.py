...
def _pending_projection_sections(
    *,
    ...
    return tee_iterator_factory(iter_pending_sections())
    ...
    *,
    ...
    if needs_analysis:
        ...
        state.analysis_resume_state = AnalysisResumeState(
            ...
            runtime_state=AnalysisResumeRuntimeState(
                state_path=None,
                state_status=state.analysis_resume_state.projection_state.runtime_state.state_status,
                reused_files=state.analysis_resume_state.projection_state.runtime_state.reused_files,
                total_files=len(file_paths_for_run),
            ),
            source=state.analysis_resume_state.projection_state.source,
            compatibility_status=state.analysis_resume_state.projection_state.compatibility_status,
            ),
        ...
        state.analysis_resume_state = AnalysisResumeState(
            projection_state=state.analysis_resume_state.projection_state,
            support_state=AnalysisResumeSupportState(
                input_state=AnalysisResumeInputState(
                    manifest_digest=execute_deps.analysis.analysis_input_manifest_digest_fn(
                        input_manifest
                    ),
                    input_witness=state.analysis_resume_state.support_state.input_state.input_witness,
                ),
                intro_state=state.analysis_resume_state.support_state.intro_state,
            ),
        )
        ...
        if import_state_paths:
            ...
            state.analysis_resume_state = AnalysisResumeState(
                projection_state=AnalysisResumeProjectionState(
                    runtime_state=AnalysisResumeRuntimeState(
                        state_path=state.analysis_resume_state.projection_state.runtime_state.state_path,
                    ...
                    }.get(next_state_status, "cold_start"),
        ...
        if state.analysis_resume_state.projection_state.compatibility_status is None:
            state.analysis_resume_state = AnalysisResumeState(
                projection_state=AnalysisResumeProjectionState(
                    runtime_state=state.analysis_resume_state.projection_state.runtime_state,
                    source=state.analysis_resume_state.projection_state.source,
                    compatibility_status="cold_start",
                ),
                support_state=state.analysis_resume_state.support_state,
            )
        state.report_runtime_state = ReportRuntimeState(
            projection_state=state.report_runtime_state.projection_state,
            checkpoint_state=ReportCheckpointState(
                section_witness_digest=_report_witness_digest(
                    input_witness=state.analysis_resume_state.support_state.input_state.input_witness,
                    manifest_digest=state.analysis_resume_state.support_state.input_state.manifest_digest,
                ),
                phase_checkpoint_state=(
                    state.report_runtime_state.checkpoint_state.phase_checkpoint_state
                ),
            ),
        )
        if report_output_path is not None:
            state.report_runtime_state = ReportRuntimeState(
                projection_state=state.report_runtime_state.projection_state,
                checkpoint_state=ReportCheckpointState(
                    section_witness_digest=state.report_runtime_state.checkpoint_state.section_witness_digest,
                    phase_checkpoint_state={},
                ),
            )
    ...
    *,
    ...
    if context.emit_phase_progress_events:
        context.emit_lsp_progress_fn(
            phase="collection",
            collection_progress={
                "completed_files": context.analysis_resume_state.projection_state.runtime_state.reused_files,
                "in_progress_files": 0,
                "remaining_files": max(
                    context.analysis_resume_state.projection_state.runtime_state.total_files
                    - context.analysis_resume_state.projection_state.runtime_state.reused_files,
                    0,
                ),
                "total_files": context.analysis_resume_state.projection_state.runtime_state.total_files,
            },
            semantic_progress=None,
            work_done=context.analysis_resume_state.projection_state.runtime_state.reused_files,
            work_total=context.analysis_resume_state.projection_state.runtime_state.total_files,
    ...
    if context.needs_analysis:
        ...
        with policy_runtime.runtime_policy_scope(runtime_policy):
            analysis = execute_deps.analysis.analyze_paths_fn(
                ...
                include_constant_smells=bool(context.report_request_state.report_path),
                include_unused_arg_smells=bool(context.report_request_state.report_path),
                include_deadness_witnesses=bool(context.report_request_state.report_path)
                ...
                include_deadline_obligations=bool(context.report_request_state.report_path) or context.lint,
                include_decision_surfaces=context.include_decisions,
                include_value_decision_surfaces=context.include_decisions,
                include_invariant_propositions=bool(context.report_request_state.report_path),
    ...
    *,
    ...
    if report_request_state.report_path:
        ...
        runtime_obligations = _incremental_progress_obligations(
            ...
            resume_payload_available=(context.analysis_resume_runtime_state.reused_files > 0),
        ...
        materialized_report = MaterializedReportArtifacts(
            resolved_sections=resolved_sections,
            pending_sections=final_pending_sections,
        )
...
@dataclass(frozen=True, init=False)
class TraceArtifactResponsePayloadBundle:
    ...
    def __init__(self, trace_artifacts: object) -> None:
        object.__setattr__(
            self,
            "aspf_trace",
            _copy_json_mapping(trace_artifacts.trace_payload),
        )
        object.__setattr__(
            self,
            "aspf_equivalence",
            _copy_json_mapping(trace_artifacts.equivalence_payload),
        )
        object.__setattr__(
            self,
            "aspf_opportunities",
            _copy_json_mapping(trace_artifacts.opportunities_payload),
        )
        object.__setattr__(
            self,
            "aspf_delta_ledger",
            _copy_json_mapping(trace_artifacts.delta_ledger_payload),
        )
        object.__setattr__(
            self,
            "aspf_state",
...
@dataclass(frozen=True, init=False)
class TimeoutResumeProgressState:
    collection_resume_payload: JSONObject
    progress_payload: JSONObject
    ...
    def __init__(
        ...
        *,
        ...
        if not collection_resume:
            object.__setattr__(self, "collection_resume_payload", collection_resume)
            object.__setattr__(self, "progress_payload", progress_payload)
            return
        ...
        object.__setattr__(self, "progress_payload", updated_progress_payload)
...
def _handle_timeout_cleanup(
    *,
    ...
    phase_checkpoint_state = context.report_runtime_state.checkpoint_state.phase_checkpoint_state
    ensure_report_sections_cache = context.ensure_report_sections_cache_fn
    ...
    try:
        ...
        timeout_resume_progress_state = TimeoutResumeProgressState(
            context=context,
            progress_payload=progress_payload,
        )
        ...
        timeout_classification_value = timeout_classification_decision(
            progress_payload=progress_payload
        )
        timeout_report_outcome = TimeoutReportOutcome(
            context=context,
            analysis_state=analysis_state,
            progress_payload=progress_payload,
            timeout_collection_resume_payload=(
        ...
        trace_artifacts = execute_deps.progress.finalize_trace_fn(
            ...
            semantic_surface_payloads={
                ...
                "_resume_source": continuation_state.resume_state.projection_state.source,
                "_resume_compatibility_status": (
                    continuation_state.resume_state.projection_state.compatibility_status
                ),
            },
            exit_code=2,
            analysis_state=analysis_state,
        ...
        if trace_artifacts is not None:
            TraceArtifactResponsePayloadBundle(trace_artifacts).apply_to_response(
...
def _build_success_response(
    *,
    ...
    if (
        continuation_state.resume_state.projection_state.runtime_state.state_path
        is not None
        or continuation_state.resume_state.projection_state.source
        != "cold_start"
        or continuation_state.resume_state.projection_state.runtime_state.reused_files
        > 0
    ):
        cache_verdict = _analysis_resume_cache_verdict(
            status=(continuation_state.resume_state.projection_state.runtime_state.state_status),
            reused_files=(continuation_state.resume_state.projection_state.runtime_state.reused_files),
            compatibility_status=(continuation_state.resume_state.projection_state.compatibility_status),
        )
        response["analysis_resume"] = {
            "checkpoint_path": (
                ...
                if continuation_state.resume_state.projection_state.runtime_state.state_path is not None
                else None
            ),
            "source": continuation_state.resume_state.projection_state.source,
            "manifest_digest": (
                continuation_state.resume_state.support_state.input_state.manifest_digest
            ),
            "reused_files": (
                continuation_state.resume_state.projection_state.runtime_state.reused_files
            ),
            "total_files": (
                continuation_state.resume_state.projection_state.runtime_state.total_files
            ),
            "remaining_files": max(
                continuation_state.resume_state.projection_state.runtime_state.total_files
                - continuation_state.resume_state.projection_state.runtime_state.reused_files,
            ...
            "status": (
                continuation_state.resume_state.projection_state.runtime_state.state_status
            ),
            "compatibility_status": (
                continuation_state.resume_state.projection_state.compatibility_status
            ),
    ...
    report_outcome = _finalize_report_and_violations(
        context=_ReportFinalizationContext(
            ...
            analysis_resume_runtime_state=(
                continuation_state.resume_state.projection_state.runtime_state
            ),
...
def execute_command_total(
    ...
    *,
    ...
    report_runtime_state = ReportRuntimeState(
        projection_state=report_projection_state,
    ...
    def _ensure_report_sections_cache() -> tuple[
        ...
        if not report_sections_cache_loaded:
            report_sections_cache, report_sections_cache_reason = (
                ...
                witness_digest=report_runtime_state.checkpoint_state.section_witness_digest,
                )
            )
    ...
    execute_deps.output.write_bootstrap_incremental_artifacts_fn(
        ...
        phase_checkpoint_state=report_runtime_state.checkpoint_state.phase_checkpoint_state,
    )
    try:
        ...
        report_runtime_state = ReportRuntimeState(
            projection_state=report_projection_state,
            checkpoint_state=report_runtime_state.checkpoint_state,
        )
        ...
        file_paths_for_run, collection_resume_payload = _prepare_analysis_resume_state(
            ...
            state=analysis_resume_preparation_state,
            runtime_state=runtime_state,
        )
        analysis_resume_state = analysis_resume_preparation_state.analysis_resume_state
        report_runtime_state = analysis_resume_preparation_state.report_runtime_state
        report_request_state = ReportRequestState(
            report_path=report_path,
        ...
        report_request_state = success_outcome.report_request_state
        report_runtime_state = report_request_state.runtime_state
