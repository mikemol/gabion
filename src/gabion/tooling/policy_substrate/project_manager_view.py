...
_README_SECTION_BEGIN = "<!-- BEGIN:project_manager_view -->"
_README_SECTION_END = "<!-- END:project_manager_view -->"
...
def _readme_action_summary(action: "ProjectManagerAction | None") -> str:
    if action is None:
        return "`<none>`"
    if action.object_id:
        return (
            f"`{action.object_id}`"
            f" via `{action.recommended_action or action.followup_family or 'followup'}`"
            f" - {_truncate_text(action.title, limit=96)}"
        )
    owner = action.owner_root_object_id or action.followup_family or "<unassigned>"
    return (
        f"`{action.followup_family or action.action_kind or 'followup'}`"
        f" via `{action.recommended_action or action.action_kind or 'action'}`"
        f" on `{owner}`"
    )
...
def render_readme_section(view: ProjectManagerView) -> str:
    lines = [
        "## Live Project Manager View",
        "",
        "_This section is auto-generated by `mise exec -- python -m scripts.policy.project_manager_view` from the current planning artifacts._",
        "",
        f"- **Repo next action:** {_readme_action_summary(view.repo_next_action)}",
    ]
    if view.repo_code_followup is not None:
        lines.append(
            f"- **Repo code followup:** {_readme_action_summary(view.repo_code_followup)}"
        )
    lines.extend(
        (
            "- **Artifacts:** "
            "[`project_manager_view.json`](artifacts/out/project_manager_view.json), "
            "[`project_manager_view.md`](artifacts/out/project_manager_view.md), "
            "[`project_manager_view.mmd`](artifacts/out/project_manager_view.mmd)",
            "",
            "| ID | Workstream | Status | Pressure | Recommended followup |",
            "| --- | --- | --- | ---: | --- |",
        )
    )
    for workstream in view.workstreams[: view.visual_limit]:
        lines.append(
            "| {object_id} | {title} | {status} | {pressure} | {followup} |".format(
                object_id=f"`{_markdown_cell(workstream.object_id)}`",
                title=_markdown_cell(_truncate_text(workstream.title, limit=72)),
                status=f"`{_markdown_cell(workstream.status)}`",
                pressure=workstream.pressure_score,
                followup=(
                    ""
                    if workstream.recommended_followup is None
                    else f"`{_markdown_cell(workstream.recommended_followup.object_id or workstream.recommended_followup.followup_family)}`"
                ),
            )
        )
    return "\n".join(lines) + "\n"
...
def _replace_readme_section(readme_text: str, section_text: str) -> str:
    if _README_SECTION_BEGIN not in readme_text or _README_SECTION_END not in readme_text:
        raise ValueError(
            "README.md is missing project manager view markers: "
            f"{_README_SECTION_BEGIN} / {_README_SECTION_END}"
        )
    before, remainder = readme_text.split(_README_SECTION_BEGIN, 1)
    _, after = remainder.split(_README_SECTION_END, 1)
    return (
        before.rstrip("\n")
        + "\n"
        + _README_SECTION_BEGIN
        + "\n"
        + section_text.rstrip()
        + "\n"
        + _README_SECTION_END
...
def _sync_readme(path: Path, view: ProjectManagerView) -> None:
    readme_text = path.read_text(encoding="utf-8")
    updated = _replace_readme_section(readme_text, render_readme_section(view))
