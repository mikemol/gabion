    ...
    *,
    ...
    issue_ids = list(
        ordered_or_sorted(
            ...
            source="src.gabion.tooling.sppf.sync_core.issue_ids",
            key=str,
        )
    )
    if not issue_ids:
...
def _run_sync_mode(
    ...
    *,
    ...
    collect_commits_fn: Callable[[str], list[CommitInfo]] = _collect_commits,
    issue_ids_from_commits_fn: Callable[[list[CommitInfo]], set[str]] = _issue_ids_from_commits,
    build_comment_fn: Callable[[str, list[CommitInfo]], str] = _build_comment,
    run_gh_fn: Callable[[list[str], bool], None] = _run_gh,
) -> int:
    check_deadline()
    rev_range = args.rev_range or default_range_fn()
    commits = collect_commits_fn(rev_range)
    ...
    for issue_id in issue_ids:
        check_deadline()
        if args.close:
            run_gh_fn(["issue", "close", issue_id, "-c", summary_comment], args.dry_run)
        elif args.comment:
            run_gh_fn(["issue", "comment", issue_id, "-b", summary_comment], args.dry_run)
        for label in labels:
            check_deadline()
            run_gh_fn(["issue", "edit", issue_id, "--add-label", str(label)], args.dry_run)
    return 0
...
def main(
    ...
    *,
    run_sppf_sync_compat_fn: Callable[[list[str]], int] | None = None,
    run_sync_mode_fn: Callable[[argparse.Namespace], int] = _run_sync_mode,
    ...
    with _deadline_scope():
        ...
        if run_sppf_sync_compat_fn is not None:
            return run_sppf_sync_compat_fn(resolved_argv)
        return run_sync_mode_fn(args)
