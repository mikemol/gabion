"""CLI entry point for `gabion aspf handoff`.

This module provides `main(argv)` for the `aspf.handoff` tooling runner key.
The underlying data layer lives in `aspf_handoff.py` and `aspf_lifecycle.py`;
this module owns only the CLI glue: argparse, JSON output, and subprocess
orchestration.
"""
...
def _parse_raw_check_args(command: Sequence[str]) -> _RawCheckArgs | None:
    ...
    while idx < len(options):
        token = options[idx]
        if token in {"--aspf-state-json", "--aspf-delta-jsonl", "--aspf-import-state"} and idx + 1 < len(options):
            value = options[idx + 1]
