...
def _collect_with_context(
    *,
    ...
    for module in batch.modules:
        visitor = _ScalarSentinelVisitor(rel_path=module.rel_path, run_context=run_context)
...
def _failure_violation(
    *,
    ...
    return Violation(
        ...
        kind=seed.kind,
        message=seed.detail,
        scalar_literal="module_failure",
        comparison_operator="module_failure",
        input_slot="module_failure",
        flow_identity=flow_identity,
        structured_hash=structured_hash,
        fiber_trace=(
        ...
        counterfactual_boundary=FiberCounterfactualBoundary(
            ...
            eliminates_violation_without_other_changes=True,
            preserves_prior_normalization=True,
            rationale="Ensure module parse/read validity before sentinel-contract analysis.",
        ),
    )
