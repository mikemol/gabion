...
@dataclass(frozen=True)
class CheckBaselineGateLintOptionBundle:
    baseline: Path | None
...
@dataclass(frozen=True)
class CheckStatusWatchOptionBundle:
...
class CheckRunOptionBundle:
    shared: CheckSharedOptionBundle
    baseline_gate_lint: CheckBaselineGateLintOptionBundle
    aspf: CheckAspfOptionBundle
    status_watch: CheckStatusWatchOptionBundle
...
@dataclass(frozen=True)
class CheckOptionSurface:
    command: str
    options: frozenset[str]


CHECK_RUN_OPTION_SURFACE = CheckOptionSurface(
    command="run",
    options=frozenset(
        {
            "baseline",
            ...
            "status_watch_run_id",
            "status_watch_branch",
            "status_watch_workflow",
            "status_watch_status",
            "status_watch_prefer_active",
            "status_watch_download_artifacts_on_failure",
            "status_watch_artifact_output_root",
            "status_watch_artifact_name",
            "status_watch_collect_failed_logs",
            "status_watch_summary_json",
        }
    ),
)
CHECK_DELTA_BUNDLE_OPTION_SURFACE = CheckOptionSurface(
    command="delta-bundle",
    options=CHECK_RUN_OPTION_SURFACE.options,
)


def _option_surface_holes(*, base: CheckOptionSurface, candidate: CheckOptionSurface) -> frozenset[str]:
    return frozenset(base.options.difference(candidate.options))


def _build_check_run_option_bundle(
    *,
    ctx: typer.Context,
    paths: list[Path],
    root: Path,
    ...
    status_watch_collect_failed_logs: bool | None = None,
    status_watch_summary_json: Path | None = None,
) -> CheckRunOptionBundle:
    return CheckRunOptionBundle(
        shared=CheckSharedOptionBundle(
            ctx=ctx,
            paths=paths,
            root=root,
...
def _delta_bundle_runtime_overrides(
    *,
    option_bundle: CheckRunOptionBundle,
    check_gate_mode: type,
    check_lint_mode: type,
) -> CheckRunOptionBundle:
    return option_bundle.with_overrides(
        baseline_gate_lint=replace(
            option_bundle.baseline_gate_lint,
            baseline=None,
            baseline_mode=None,
...
class CheckRunDecisionProtocol:
    ...
    def resolve(self) -> CheckRunDecisionResolution:
        baseline = self.option_bundle.baseline_gate_lint.baseline
        baseline_write = False
        baseline_mode = self.option_bundle.baseline_gate_lint.baseline_mode
        ...
        if self.build_status_watch_options_fn is not None:
            watch_bundle = self.option_bundle.status_watch
            status_watch_options = self.build_status_watch_options_fn(
                status_watch=watch_bundle.status_watch,
                run_id=watch_bundle.status_watch_run_id,
                branch=watch_bundle.status_watch_branch,
                workflow=watch_bundle.status_watch_workflow,
                status=watch_bundle.status_watch_status,
                prefer_active=watch_bundle.status_watch_prefer_active,
                download_artifacts_on_failure=watch_bundle.status_watch_download_artifacts_on_failure,
                artifact_output_root=watch_bundle.status_watch_artifact_output_root,
                artifact_name=watch_bundle.status_watch_artifact_name,
                collect_failed_logs=watch_bundle.status_watch_collect_failed_logs,
                summary_json=watch_bundle.status_watch_summary_json,
            )
...
def _check_run_runtime_kwargs_from_bundle(
    *,
    ...
    shared = option_bundle.shared
    baseline_gate_lint = option_bundle.baseline_gate_lint
    aspf = option_bundle.aspf
    return {
        "ctx": shared.ctx,
        "paths": shared.paths,
        ...
        "decision_snapshot": shared.decision_snapshot,
        ...
        "allow_external": shared.allow_external,
        "strictness": str(shared.strictness.value),
        "analysis_budget_checks": shared.analysis_budget_checks,
        "gate": baseline_gate_lint.gate,
        "lint_mode": baseline_gate_lint.lint_mode,
        "lint_jsonl_out": baseline_gate_lint.lint_jsonl_out,
        "lint_sarif_out": baseline_gate_lint.lint_sarif_out,
        "aspf_trace_json": aspf.aspf_trace_json,
        "aspf_import_trace": aspf.aspf_import_trace,
        "aspf_equivalence_against": aspf.aspf_equivalence_against,
        "aspf_opportunities_json": aspf.aspf_opportunities_json,
        "aspf_state_json": aspf.aspf_state_json,
        "aspf_delta_jsonl": aspf.aspf_delta_jsonl,
        "aspf_import_state": aspf.aspf_import_state,
        "aspf_semantic_surface": aspf.aspf_semantic_surface,
...
def register_check_run_command(
    *,
    ...
    def check_run(
        ...
        option_bundle = _build_check_run_option_bundle(
            ctx=ctx,
            paths=paths,
            root=root,
            config=config,
...
def register_check_delta_bundle_command(
    *,
    ...
    missing_options = _option_surface_holes(
        base=CHECK_RUN_OPTION_SURFACE,
        candidate=CHECK_DELTA_BUNDLE_OPTION_SURFACE,
    )
    if missing_options:
        missing_options_csv = ", ".join(sorted(missing_options))
        raise typer.BadParameter(
            f"delta-bundle option surface drift: missing promoted options: {missing_options_csv}"
        )
    ...
    def check_delta_bundle(
        ...
        runtime_kwargs = _check_run_runtime_kwargs_from_bundle(
            option_bundle=_delta_bundle_runtime_overrides(
                option_bundle=option_bundle,
                check_gate_mode=check_gate_mode,
                check_lint_mode=check_lint_mode,
            ),
