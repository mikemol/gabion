...
def emit_timeout_profile_artifacts(
    ...
    *,
    ...
    timeout_context = json_mapping_optional(result.get("timeout_context"))
    if timeout_context is None:
        return
    profile = json_mapping_optional(timeout_context.get("deadline_profile"))
    if profile is None:
        return
...
def emit_analysis_resume_summary(result: JSONObject) -> None:
    resume = json_mapping_optional(result.get("analysis_resume"))
    if resume is None:
        return
