...
doc_reviewed_as_of:
  ...
  glossary.md#partition_witness: 2
...
doc_review_notes:
  ...
  glossary.md#decision_surface: Reviewed glossary.md#decision_surface rev1 (decision
    surface tier boundary semantics).
  glossary.md#ambiguity_set: Reviewed glossary.md#ambiguity_set rev2 (ambiguity sets
    are emergent from CallCandidate alts on call suites).
  glossary.md#partition_witness: Reviewed glossary.md#partition_witness rev2 (partition
    witnesses attach to call suites; ambiguity set is virtual).
  ...
  AGENTS.md#agent_obligations: Agent obligations unchanged; suite carrier remains
    audit-driven.
  ...
  in/in-29.md#in_in_29: Reviewed in/in-29.md rev16 (evidence dominance + updated in-23/24/25
    pins); artifact paths aligned.
...
doc_section_reviews:
  in_in_30:
    ...
    glossary.md#decision_surface:
      ...
      self_version_at_review: 4
      outcome: no_change
      note: Reviewed glossary.md#decision_surface rev1 (decision surface tier boundary
        semantics).
    glossary.md#ambiguity_set:
      dep_version: 2
      self_version_at_review: 4
      ...
      note: Reviewed glossary.md#ambiguity_set rev2 (ambiguity sets are emergent from
        CallCandidate alts on call suites).
    glossary.md#partition_witness:
      dep_version: 2
      self_version_at_review: 4
      outcome: no_change
      note: Reviewed glossary.md#partition_witness rev2 (partition witnesses attach
        to call suites; ambiguity set is virtual).
    CONTRIBUTING.md#contributing_contract:
      dep_version: 2
      self_version_at_review: 4
      outcome: no_change
    ...
    in/in-29.md#in_in_29:
      ...
      self_version_at_review: 4
      outcome: no_change
      note: Reviewed in/in-29.md rev16 (evidence dominance + updated in-23/24/25 pins);
        artifact paths aligned.
---
...
- **Implemented**
  - `src/gabion/analysis/legacy_dataflow_monolith.py::_materialize_ambiguity_suite_agg_spec` and `_materialize_ambiguity_virtual_set_spec` now materialize suite-scoped ambiguity projections (`ambiguity_suite_agg`, `ambiguity_virtual_set`) for legacy-compatible reporting (SPPF: in-30 checklist row, GH-88).
  - `src/gabion/analysis/legacy_dataflow_monolith.py::_materialize_suite_order_spec` plus `src/gabion/analysis/projection_registry.py` (`name="suite_order"`) provide suite-order SpecFacet materialization used by projection reports (SPPF: in-30 checklist row, GH-85).
  ...
  - `src/gabion/analysis/legacy_dataflow_monolith.py::_collect_deadline_obligations` now attributes recursive deadline obligations at loop-suite scope (outer loop remains violating when only inner loop is checked), and emits stable SuiteSite identities reused across deadline artifacts (`site.suite_id`, `site.suite_kind`) (SPPF: in-30 checklist row, GH-85; acceptance: `tests/test_legacy_dataflow_monolith_edges.py::test_deadline_nested_recursion_loop_attributes_inner_only`, `...::test_deadline_suite_identity_stable_across_runs`, `...::test_deadline_obligations_emit_suite_metadata_from_forest`).
- **Blocked/Deferred**
...
- Phase 1: migrate ambiguity witnesses to call suite scope (with aggregation spec).
...
## Proof-by-Construction Mode (never())
We enforce *proof by construction* by converting tolerated `None` values into
`never()` obligations under a strict mode. This is how we surface hidden
assumptions and force optionality to become explicit.

### Mechanism
- Add `require_not_none(...)` and `proof_mode()` in `gabion.invariants`.
- When `GABION_PROOF_MODE=strict`, `require_not_none(None, ...)` raises `never()`.
- Non-strict runs preserve legacy defensive behavior (still return None).

### Plan (ratchet)
1. Replace `is None` / `is not None` guards at artifact boundaries with
   `require_not_none(...)`.
2. Run strict mode to surface latent `None` paths.
3. Fix by construction (remove `None` sources) or document explicit optionality.
...
| AmbiguitySet (virtual) | CallCandidate alts on SuiteSite; AmbiguitySet is projection |
...
* Ambiguity report:
  ...
  * group CallCandidate alts by SuiteSite (count > 1 ⇒ AmbiguitySet)
...
* Emit CallCandidate alts at SuiteSite(call) level (AmbiguitySet is virtual).
* Keep existing function-scoped ambiguity output.
* Preserve ambiguity witness observation multiplicity during analysis/reporting; do not collapse repeated observations that arise from distinct traversal evidence.
...
Candidate ordering is erased, candidate identity is not; repeated witness observations remain a signal of analyzer/code smell until root causes are resolved.
