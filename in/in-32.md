...
doc_review_notes:
  POLICY_SEED.md#policy_seed: "Reviewed POLICY_SEED.md rev2 (mechanized governance); no conflicts with this document's scope."
  ...
  CONTRIBUTING.md#contributing_contract: "Reviewed CONTRIBUTING.md rev2; no conflicts."
  README.md#repo_contract: "Reviewed README.md rev2; phased migration preserves existing tooling."
  AGENTS.md#agent_obligations: "Reviewed AGENTS.md#agent_obligations rev2; agent must enforce determinism and correctness at each phase."
  in/in-30.md#in_in_30: "Re-reviewed in/in-30.md#in_in_30 rev4; SuiteSite and loop-suite semantics remain compatible with this document's finite-carrier migration framing."
...
doc_section_reviews:
  in_in_32:
    POLICY_SEED.md#policy_seed:
      ...
      outcome: no_change
      note: "Reviewed POLICY_SEED.md section rev2; no conflicts."
    glossary.md#aspf:
      dep_version: 1
      self_version_at_review: 6
      outcome: no_change
...
1. **MS1 — ASPF classed subtree reuse (`src/gabion/analysis/dataflow/io/dataflow_structure_reuse.py`, `src/gabion/analysis/core/structure_reuse_classes.py`)**
   - Invariant: subtree reuse grouping keys are derived from canonical ASPF structure classes, not ad-hoc payload hashes.
   - Test gate: `tests/gabion/analysis/dataflow_s2/dataflow_structure_reuse_cases.py::test_compute_structure_reuse_detects_repeated_subtrees` asserts `aspf_structure_class` payloads exist on reuse groups.

2. **MS2 — Fingerprint/rewrite witness coupling (`src/gabion/analysis/dataflow/engine/dataflow_decision_surfaces.py`, `src/gabion/analysis/dataflow/engine/dataflow_fingerprint_helpers.py`)**
   - Invariant: every generated rewrite plan carries explicit witness obligations plus at least one non-regression gate.
   - Test gate: `tests/gabion/analysis/dataflow_s2/test_dataflow_decision_surfaces_module.py::test_decision_surface_summaries_and_plans_cover_edges` and `tests/gabion/analysis/misc_s3/test_rewrite_plan_verification.py::test_verify_rewrite_plan_enforces_witness_obligations_non_regression`.

3. **MS3 — ASPF mutation log shadow replay (`src/gabion/analysis/aspf/aspf_mutation_log.py`)**
   ...
   - Test gate: `tests/gabion/analysis/aspf/test_aspf_mutation_log.py`.
