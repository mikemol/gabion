...
* Invariant propositions now carry deterministic `invariant_id`, confidence, and evidence-key metadata in analysis outputs and resume payloads.
* Synthesis planning now includes a framework-agnostic `property_hook_manifest` keyed by invariant IDs.
* Hook emission is confidence-gated and traceable via `source_invariant_evidence_keys`.
* Optional Hypothesis skeleton snippets are available behind a synthesis flag; manifest output remains the default.
...
Status: partial (invariant proposition + property-hook manifest integration landed; dependent-type Agda backend remains planned)