...
doc_review_notes:
  POLICY_SEED.md#policy_seed: "Reviewed POLICY_SEED.md section rev2 (mechanized governance default and correction-unit validation stack); no conflicts with this document's scope."
  ...
  CONTRIBUTING.md#contributing_contract: "Reviewed CONTRIBUTING.md section rev2 (dual-sensor cadence and correction-unit validation stack); no conflicts with this document's scope."
  README.md#repo_contract: "Reviewed README.md section rev2 (repo contract scope/status framing); no conflicts with this document's scope."
  AGENTS.md#agent_obligations: "Agent obligations remain consistent with evidence-first carrier work."
...
doc_section_reviews:
  in_in_23:
    POLICY_SEED.md#policy_seed:
      ...
      outcome: no_change
      note: "Reviewed POLICY_SEED.md section rev2 (mechanized governance default and correction-unit validation stack); no conflicts with this document's scope."
    glossary.md#aspf:
      dep_version: 1
      self_version_at_review: 1
      outcome: no_change
    ...
    CONTRIBUTING.md#contributing_contract:
      ...
      self_version_at_review: 1
      outcome: no_change
      note: "Reviewed CONTRIBUTING.md section rev2 (dual-sensor cadence and correction-unit validation stack); no conflicts with this document's scope."
    README.md#repo_contract:
      dep_version: 2
      self_version_at_review: 1
      outcome: no_change
      note: "Reviewed README.md section rev2 (repo contract scope/status framing); no conflicts with this document's scope."
    AGENTS.md#agent_obligations:
      dep_version: 2
      self_version_at_review: 1
      outcome: no_change
      note: "Agent obligations remain consistent with evidence-first carrier work."
---
<a id="in_in_23"></a>
...
**Adopted (completed landing).**
The ASPF carrier formalization is landed: deterministic synth/provenance artifacts, reloadable basis handling, and snapshot plumbing are implemented and exercised by tests.

Mechanical anchors (module/test):

* `src/gabion/analysis/dataflow/engine/dataflow_fingerprint_helpers.py::_compute_fingerprint_provenance`
* `src/gabion/analysis/dataflow/engine/dataflow_fingerprint_helpers.py::_compute_fingerprint_synth`
* `src/gabion/analysis/core/type_fingerprints.py::synth_registry_payload`
* `src/gabion/analysis/core/type_fingerprints.py::build_synth_registry_from_payload`
* `tests/gabion/analysis/type/test_type_fingerprints.py::test_build_fingerprint_registry_deterministic_assignment`
* `tests/gabion/analysis/type/test_type_fingerprints.py::test_synth_registry_payload_roundtrip`
* `tests/gabion/analysis/misc_s2/test_fingerprint_warnings.py::test_fingerprint_provenance_emits_entries`
* `scripts/audit_snapshot.sh` and `scripts/latest_snapshot.sh` (artifact capture/selectors)
...
Normative pointers (explicit): [POLICY_SEED.md#policy_seed](POLICY_SEED.md#policy_seed), [glossary.md#aspf](glossary.md#aspf), [glossary.md#hash_consing](glossary.md#hash_consing), [glossary.md#deadness_witness](glossary.md#deadness_witness), [CONTRIBUTING.md#contributing_contract](CONTRIBUTING.md#contributing_contract), [README.md#repo_contract](README.md#repo_contract), [AGENTS.md#agent_obligations](AGENTS.md#agent_obligations).

---

## 0.1 HOL via Internment (Proof Sketch)

**Claim:** higher‑order evaluation reduces to **construction + internment**
when the carrier is ASPF‑interned and normalization includes the required
reductions.
...
- ASPF‑interned carriers yield a **finite, enumerated domain**.
- ProjectionSpec yields counterexample/witness projections.
- Other carriers are **interned into ASPF**, so their evaluation inherits ASPF
  normalization (no separate evaluator).

**Reduction expressibility:** any required reduction is added as an ASPF
normalization rule on the carrier’s syntax nodes; the act of constructing and
interning those nodes performs the evaluation.
...
## 0.2 Embedded Logical Systems (Already Present)

Because ASPF internment yields **finite, enumerated carriers** and normalization
is intrinsic to internment, the following systems are **already embedded**
without adding new primitives:

1. **Finite‑domain intuitionistic FOL**
   - `∀/∃` reduce to finite conjunction/disjunction over the carrier.
2. **Relational algebra / SQL (SPJ + Aggregate fragment)**
   - `select`, `project`, `count_by`, `sort`, `limit` are native.
   - Joins are already present when carriers emit **pre‑joined relations**.
...
4. **Simply‑typed lambda calculus (extensional equality)**
   - β‑reduction is a normalization rule; evaluation is internment.
5. **HOL over finite domains (restricted)**
   - Higher‑order syntax is evaluable when its reductions are normalization rules.

These embeddings are **extensional**: they depend on carrier emission and
normalization, not on additional evaluator primitives.
...
* `src/gabion/analysis/dataflow/engine/dataflow_pipeline.py` and `src/gabion/analysis/dataflow/engine/dataflow_fingerprint_helpers.py` produce report + artifacts
...
* I₁.2 **Comparability**: outputs are meaningful under diff
  ...
  * I₁.2.d each enforced sort discloses comparator/key semantics (including non-lexical tuple components) so canonicalization remains auditable
