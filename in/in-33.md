...
doc_review_notes:
  ...
  AGENTS.md#agent_obligations: "Reviewed AGENTS.md#agent_obligations rev2; boundary discipline and policy surfacing remain aligned with this step."
  CONTRIBUTING.md#contributing_contract: "Reviewed CONTRIBUTING.md#contributing_contract rev2; correction-unit and policy gate expectations remain aligned."
  README.md#repo_contract: "Reviewed README.md#repo_contract rev2; scope and status framing remain compatible with staged PatternSchema rollout."
...
doc_section_reviews:
  in_in_33:
    POLICY_SEED.md#policy_seed:
      ...
      outcome: no_change
      note: "Policy seed section v2 still enforces mechanized execution gates that this proposal preserves."
    glossary.md#contract:
      dep_version: 1
      self_version_at_review: 1
      outcome: no_change
...
- **Implemented**
  - `src/gabion/analysis/projection/pattern_schema.py::PatternSchema` now emits a shared cross-axis schema id (`schema:*`) via one normalization path and preserves the prior axis-scoped id as `legacy_schema_id` for compatibility annotation (SPPF: `docs/sppf_checklist.md#in-33-pattern-schema-unification`).
  - `src/gabion/analysis/projection/pattern_schema.py::mismatch_residue_payload` defines a single residue payload contract (`schema_contract=pattern_schema.v2`) reused by dataflow + execution pattern residue diagnostics (SPPF: `docs/sppf_checklist.md#in-33-pattern-schema-unification`).
  - `src/gabion/analysis/projection/pattern_schema_projection.py::pattern_schema_snapshot_entries` now serializes `schema_contract` + `legacy_schema_id` and enforces deterministic residue ordering in emitted schema artifacts (SPPF: `docs/sppf_checklist.md#in-33-pattern-schema-unification`).
  - `src/gabion/analysis/projection/pattern_schema_projection.py::execution_pattern_instances` and `bundle_pattern_instances` emit stable near-miss residue diagnostics via the same schema contract for both axes (SPPF: `docs/sppf_checklist.md#in-33-pattern-schema-unification`).
- **Open**
  - Execution-pattern rule coverage remains intentionally narrow in `src/gabion/analysis/projection/pattern_schema_projection.py::EXECUTION_PATTERN_RULES`, so many callable/dataflow shapes still bypass PatternSchema unification (SPPF: `docs/sppf_checklist.md#in-33-pattern-schema-unification`).
- **Blocked/Deferred**
...
- Execution pattern mining scaffold (`pattern_schema_projection.py` execution-pattern rules).
...
- **Location:** `src/gabion/analysis/projection/pattern_schema_projection.py`
  - Add schema types near `_ExecutionPatternMatch` and `_ExecutionPatternRule`.
  - Add `pattern_schema_matches(...)` to return `PatternInstance`.
  - Add `pattern_schema_suggestions_from_instances(...)` to unify reporting.
  - Add `pattern_schema_residue_entries(...)` emission to report helpers.
