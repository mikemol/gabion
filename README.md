...
- **Repo next action:** `workspace_orphan_resolution` via `adopt_attribute_stage_validate_commit_orphaned_change` on `WS-SEED:scripts.misc`
...
| --- | --- | --- | ---: | --- |
| `CSA-RGC` | Declarative registry convergence and wrapper/package inversion cleanup | `in_progress` | 225 | `CSA-RGC-TP-002` |
| `CSA-IGM` | Ingress/merge witness, remap, and non-Python ASPF adapter rollout | `in_progress` | 99 | `CSA-IGM-TP-002` |
...
| `PRF` | Policy rule markdown-frontmatter migration | `landed` | 2 | `documentation_alignment` |
