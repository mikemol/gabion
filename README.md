...
doc_review_notes:
  ...
  AGENTS.md#agent_obligations: "Reviewed AGENTS.md#agent_obligations rev37/section v2 (required validation stack, practical quick-start guidance, and ci_watch failure-bundle triage guidance remain aligned)."
...
- **Repo next action:** `workspace_preservation` via `adopt_stage_validate_commit_graph_participating_change` on `PRF`
- **Repo code followup:** `PSF-007-TP-005` via `structural_cut` - projection_exec_plan.py planning surfaces
- **Artifacts:** [`project_manager_view.json`](artifacts/out/project_manager_view.json), [`project_manager_view.md`](artifacts/out/project_manager_view.md), [`project_manager_view.mmd`](artifacts/out/project_manager_view.mmd)

| ID | Workstream | Status | Pressure | Recommended followup |
| --- | --- | --- | ---: | --- |
| `CSA-RGC` | Declarative registry convergence and wrapper/package inversion cleanup | `in_progress` | 289 | `CSA-RGC-TP-002` |
| `CSA-IGM` | Ingress/merge witness, remap, and non-Python ASPF adapter rollout | `in_progress` | 163 | `CSA-IGM-TP-002` |
| `CSA-IDR` | Identity/rendering separation and structural interning rollout | `in_progress` | 87 | `CSA-IDR-TP-003` |
| `PSF-007` | Cut over legacy adapters and retire semantic_carrier_adapter boundaries | `in_progress` | 76 | `PSF-007-TP-005` |
| `CSA-IVL` | Impact-velocity and planner turnaround optimization | `in_progress` | 71 | `CSA-IVL-TP-002` |
| `PRF` | Policy rule markdown-frontmatter migration | `in_progress` | 13 | `PRF-TP-005` |
<!-- END:project_manager_view -->
