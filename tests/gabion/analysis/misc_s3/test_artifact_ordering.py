from gabion.analysis.foundation.artifact_ordering import (
    canonical_count_summary_items, canonical_doc_scope, canonical_field_display_parts, canonical_mapping_keys, canonical_protocol_specs, canonical_string_values)
...
def test_canonical_doc_scope_uses_default_for_empty_cleaned_scope() -> None:
    assert tuple(canonical_doc_scope(["", " ", "\t"])) == ("repo", "artifacts")
    assert tuple(canonical_doc_scope([" docs ", "repo"])) == ("docs", "repo")
...
def test_canonical_protocol_specs_and_count_items_are_ordered() -> None:
    ...
    ordered = tuple(canonical_protocol_specs(protocols))
...
def test_canonical_string_values_and_mapping_keys_sort() -> None:
    assert tuple(canonical_string_values([3, "1", 2])) == ("1", "2", "3")
    assert tuple(canonical_mapping_keys({"b": 1, "a": 2})) == ("a", "b")
