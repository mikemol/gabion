...
def test_collect_test_files_includes_cases_modules(tmp_path: Path) -> None:
    ...
    wrapper = tests_dir / "test_merge.py"
    wrapper.write_text("from .merge_cases import *  # noqa: F401,F403\n", encoding="utf-8")
    cases = tests_dir / "merge_cases.py"
    cases.write_text("def test_merge_case():\n    assert True\n", encoding="utf-8")

    files = test_evidence._collect_test_files(
        [tmp_path / "tests"],
        root=root,
        exclude=set(),
    )
    rel_files = {str(path.relative_to(root)) for path in files}
    assert "tests/gabion/synthesis/test_merge.py" in rel_files
    assert "tests/gabion/synthesis/merge_cases.py" in rel_files
...
def test_extract_file_evidence_uses_legacy_path_alias_and_qualname_compat(
    ...
    root = tmp_path
    tests_dir = tmp_path / "tests" / "gabion" / "analysis"
    tests_dir.mkdir(parents=True)
    source = tests_dir / "dataflow_raw_runtime_cases.py"
    source.write_text(
        "\n".join(
            [
                "# gabion:evidence E:call_footprint::tests/test_dataflow_run.py::test_legacy_dataflow_monolith_case",
                "def test_legacy_dataflow_monolith_case():",
                "    assert True",
            ]
        )
        + "\n",
    ...
    entries = test_evidence._extract_file_evidence(source, root)
    assert len(entries) == 1
    assert entries[0].test_id == "tests/test_dataflow_run.py::test_dataflow_audit_case"
...
def test_extract_file_evidence_uses_path_alias_for_prefixed_module_names(
    ...
    tests_dir = tmp_path / "tests" / "gabion" / "synthesis"
    tests_dir.mkdir(parents=True)
    source = tests_dir / "test_naming.py"
    source.write_text(
        "\n".join(
            [
                "def test_suggest_name_uses_frequency():",
                "    assert True",
            ]
        )
        + "\n",
    ...
    entries = test_evidence._extract_file_evidence(source, root)
    assert len(entries) == 1
    assert entries[0].test_id == "tests/test_synthesis_naming.py::test_suggest_name_uses_frequency"
...
def test_duplicate_detection_applies_after_legacy_path_canonicalization(
    ...
    root = tmp_path
    tests_dir = tmp_path / "tests"
    moved_dir = tests_dir / "gabion" / "synthesis"
    moved_dir.mkdir(parents=True)
    legacy_file = tests_dir / "test_synthesis_merge.py"
    moved_file = moved_dir / "merge_cases.py"
