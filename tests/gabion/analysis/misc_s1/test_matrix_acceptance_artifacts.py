...
def _load():
    repo_root = REPO_ROOT
    from gabion.analysis.dataflow.engine import dataflow_indexed_file_scan
...
def _run_with_artifacts(
    ...
    *,
    ...
    assert dataflow_runtime.run(argv) == 0
...
def test_matrix_artifacts_are_deterministic_and_have_required_fields(tmp_path: Path) -> None:
    dataflow_indexed_file_scan, dataflow_runtime = _load()
    ...
    verification = dataflow_indexed_file_scan.verify_rewrite_plan(
        rewrite_plans[0],
