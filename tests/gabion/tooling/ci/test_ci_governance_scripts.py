...
from scripts.ci import ci_controller_drift_gate as drift_gate
            ...
            "jobs": {
                "checks": {
                    "steps": [
                        ...
                        {"run": "python scripts/ci/ci_finalize_dataflow_outcome.py --terminal-exit 0"},
                    ]
                }
            }
        },
...
def test_controller_audit_detects_contradictory_anchors_across_declared_normative_docs(
    ...
    policy.write_text(
        "\n".join(
            (
                ...
                "- `controller-anchor: CD-999 | doc: POLICY_SEED.md#change_protocol | sensor: contradictory_anchors_across_normative_docs | check: scripts/governance/governance_controller_audit.py | severity: high`",
            )
        )
        + "\n",
        encoding="utf-8",
    ...
    (tmp_path / "docs" / "governance_control_loops.md").write_text(
        "- `controller-anchor: CD-999 | doc: POLICY_SEED.md#change_protocol | sensor: contradictory_anchors_across_normative_docs | check: scripts/governance/governance_controller_audit.py | severity: medium`\n",
        encoding="utf-8",
    )
