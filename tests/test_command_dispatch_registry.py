...
def test_direct_dispatch_registry_sorted_and_complete() -> None:
    ...
    assert direct_dispatch.direct_executor(command_ids.CHECK_COMMAND) is server.execute_command
    assert direct_dispatch.direct_executor(command_ids.DATAFLOW_COMMAND) is server.execute_command
