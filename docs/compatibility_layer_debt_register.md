...
| `src/gabion/analysis/dataflow/engine/dataflow_indexed_file_scan.py` + `src/gabion/analysis/dataflow/engine/dataflow_indexed_file_scan_alias_inventory.py` + owner-domain alias adapters | Maintainer (unassigned) | Temporary compatibility boundary preserves the legacy monolith import path through explicit owner-domain alias adapters and retirement telemetry while remaining external importers finish cutover. | `CU-SCC-FINAL-1` | `2026-03-31` | Retarget remaining consumers to direct owner modules, then delete the compatibility facade, alias inventory contract, and owner-domain adapter modules together. | Open |
