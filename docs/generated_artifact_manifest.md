...
| `docflow_packetization_bundle` | `artifacts/out/docflow_warning_doc_packets.json`<br>`artifacts/out/docflow_warning_doc_packet_summary.json` | `json` | command: gabion policy docflow-packetize | always - emitted on normal packetization runs | `mise exec -- python -m gabion policy docflow-packetize` | `gabion policy docflow-packet-enforce`<br>`CI audit workflow` | Packetized warning rows and packet summary for the docflow closed loop. |
| `docflow_packet_enforcement_bundle` | `artifacts/out/docflow_packet_enforcement.json`<br>`artifacts/out/docflow_packet_debt_ledger.json` | `json` | command: gabion policy docflow-packet-enforce | always - emitted on normal packet-enforcement runs | `mise exec -- python -m gabion policy docflow-packet-enforce --check` | `CI audit workflow`<br>`governance debt review` | Enforcement result plus age-bounded debt ledger. |
| `controller_drift_audit` | `artifacts/out/controller_drift.json` | `json` | command: gabion governance controller-audit | always - emitted on controller-drift audit runs | `mise exec -- python -m gabion governance controller-audit --out artifacts/out/controller_drift.json` | `scripts/ci/ci_controller_drift_gate.py`<br>`invariant graph ingress` | Second-order controller-loop state artifact. |
...
| `governance_telemetry_bundle` | `artifacts/out/governance_telemetry.json`<br>`artifacts/out/governance_telemetry_history.json`<br>`artifacts/audit_reports/governance_telemetry.md`<br>`artifacts/audit_reports/ci_step_timings.json` | `json+md` | workflow_step: .github/workflows/ci.yml audit job governance telemetry emit | always - emitted on normal CI audit runs | `mise exec -- python -m gabion governance telemetry-emit --run-id local --timings artifacts/audit_reports/ci_step_timings.json --junit artifacts/test_runs/junit.xml --local-ci-contract artifacts/out/local_ci_repro_contract.json --observability artifacts/audit_reports/observability_violations.json --history artifacts/out/governance_telemetry_history.json --json-out artifacts/out/governance_telemetry.json --md-out artifacts/audit_reports/governance_telemetry.md` | `uploaded CI artifacts`<br>`governance trend review` | Step timings remain an input carrier for telemetry, and governance telemetry history now also owns the bounded delivery-flow blocker snapshots used to derive planner-facing delivery-flow summaries. |
...
| Artifact ID | Path(s) | Format | Emitted by | Trigger | Regeneration | Primary consumers | Notes |
| --- | --- | --- | --- | --- | --- | --- | --- |
| `test_evidence_index` | `out/test_evidence.json` | `json` | command: gabion repo extract-test-evidence | always - emitted on normal evidence-index refresh runs | `mise exec -- python -m gabion repo extract-test-evidence --root . --tests tests --out out/test_evidence.json` | `invariant graph ingress`<br>`impact and obsolescence tooling` | Canonical test-evidence carrier. |
| `test_behavior_index` | `out/test_behavior.json` | `json` | command: gabion repo extract-test-behavior | always - emitted on normal behavior-index refresh runs | `mise exec -- python -m gabion repo extract-test-behavior --root . --tests tests --out out/test_behavior.json` | `behavior-index drift checks` | Canonical test-behavior carrier. |
...
| `dataflow_terminal_outcome_bundle` | `artifacts/audit_reports/dataflow_terminal_outcome.json` | `json` | runtime_path: run-dataflow-stage terminal outcome projection path | conditional - emitted when run-dataflow-stage completes with at least one stage result | `mise exec -- python -m gabion --carrier direct --timeout 130000000000000ns run-dataflow-stage --debug-dump-interval-seconds 60 --stage-strictness-profile "run=high"` | `invariant graph ingress`<br>`DGR synthetic root`<br>`audit review` | Structured terminal outcome carrier for local and CI-shaped dataflow-stage runs. |
| `dataflow_obligation_trace_bundle` | `artifacts/out/obligation_trace.json` | `json` | runtime_path: run-dataflow-stage obligation trace projection path | conditional - emitted when run-dataflow-stage completes with obligation-trace projection enabled | `mise exec -- python -m gabion --carrier direct --timeout 130000000000000ns run-dataflow-stage --debug-dump-interval-seconds 60 --stage-strictness-profile "run=high"` | `invariant graph ingress`<br>`DGR synthetic root`<br>`audit review` | Structured obligation-trace carrier used for local dataflow readiness and timeout-contract inspection. |
