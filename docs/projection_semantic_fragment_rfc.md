...
- `wedge(surface=projection_fiber)` now compiles through that same typed
  lowering/compiler path via a registered `projection_fiber_context_wedge`
...
- the same path is narrower again after that: `_input_witness_payloads(...)`
  is now treated as an ordinary internal witness-shaping helper rather than a
...
- and narrower again after that: `_naturality_payload(...)` is now treated as
  an ordinary internal naturality-witness shaping helper rather than a graded
  carrier seam, so the remaining temporary grading on `semantic_fragment.py`
  stays concentrated on the real row-closure, canonicalization, and
  obligation materialization surfaces
- and narrower again after that: `_synthesized_witness_payloads(...)` is
  collapsed back into `reflect_projection_fiber_witness(...)`, so that
  temporary work-growth grading stays on the real reflection carrier
  entrypoint instead of on a second witness-assembly wrapper
- the compile side is narrower too: `_input_witness_kinds(...)`,
  `_synthesized_witness_kinds(...)`, `_boundary_kinds(...)`,
  `_transform_ops(...)`, and `_witness_trace(...)` are now collapsed back into
  the already-graded `compile_projection_fiber_*` entrypoints, so temporary
...
- and narrower again after that: `_obligation_payloads(...)` is now treated as
  ...
  carrier seam, so the remaining temporary grading on `semantic_fragment.py`
  stays concentrated on the real row-closure and canonicalization surfaces
- and narrower again after that: the private `_normalize_mapping(...)` and
  `_normalize_list(...)` helper seams are collapsed back into
...
- and narrower again after that: the private `_stable_unique_objects(...)`
  helper seam is collapsed back into `close_canonical_semantic_row(...)`, so
  the remaining temporary grading on `semantic_fragment.py` stays concentrated
  on the real row-closure, recursive canonicalization, and stable-key
  materialization surfaces
...
- the executor-side temporary grading is now narrower too: `projection_exec.py`
  ...
  on that path is concentrated on the real typed execution surfaces plus the
  still-explicit sort/canonical-group-reference helpers they depend on
...
- `projection_exec.py` still executes the legacy row pipeline as a
  ...
  pure `ProjectionSpec` planning now lives in `projection_exec_plan.py`, the
  dedicated `projection_exec_ingress.py` adapter module is gone, and
  production code no longer carries a convenience `apply_spec(...)` wrapper at
  all
...
compose planner output back through a production convenience wrapper.
