diff --git a/docs/audits/connectivity_synergy_audit.md b/docs/audits/connectivity_synergy_audit.md
new file mode 100644
index 0000000000000000000000000000000000000000..1482ab7fa86a84046ff71618f2aaf4386a53128d
--- /dev/null
+++ b/docs/audits/connectivity_synergy_audit.md
@@ -0,0 +1,172 @@
+---
+doc_revision: 4
...
+
+## Suggested reduction roadmap (ordered)
+
+1. **Define ASPF adapter contract for non-Python substrates** (markdown/frontmatter/yaml first).
+2. **Converge governance/policy registries into declarative carriers** (frontmatter/YAML source of truth).
+3. **Unify policy scanner dependencies** (remove package -> scripts dynamic import inversion).
+4. **Extract protocol bundles from `command_orchestrator`** (progress emission, result projection, artifact routing).
+5. **Extract shared post-phase/report decision surfaces from analysis core** into reusable carriers.
+6. **Normalize script wrappers to one launcher template/manifest** and enforce via policy check.
+
+## Expected outcome
+
