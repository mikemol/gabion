...
doc_reviewed_as_of:
  ...
  docs/projection_semantic_fragment_rfc.md#projection_semantic_fragment_rfc: 76
...
doc_review_notes:
  ...
  docs/projection_semantic_fragment_rfc.md#projection_semantic_fragment_rfc: "Reviewed the implementation RFC rev76 and kept the queue aligned with policy_check-owned continuation artifacts, direct semantic-summary sourcing, duplicate suite-side queue ownership removal, duplicate suite-payload semantic-summary embedding removal, carrier-owned lattice witness serialization, source-artifact queue naming, child-owned skip/fail-closed artifact ownership, wrapper reuse of preexisting child-owned canonical artifacts, payload-first hotspot queue generation, generic source-artifact hotspot queue ingress, generic hotspot queue source metadata naming, removal of the queue sidecar policy-check supplement contract, explicit hotspot source-artifact CLI ingress, removal of the wrapper-only top-level policy_check summary-decoder branch, removal of the stale top-level summary decoder fallback, explicit policy-scanner-suite artifact-path ownership, generic cached policy-results normalization, direct semantic-field hotspot queue reporting, canonical-only shared summary-helper ingestion, removal of the redundant policy-results alias entrypoint, removal of suite-cache hash projection from hotspot queue source metadata, removal of child-owned policy-result persistence from the wrapper cache artifact, replacement of nested `policy_results.policy_check` semantic ingress with direct `projection_fiber_semantics` on suite runtime payloads, removal of full child-policy payload retention from the runtime suite result carrier, removal of wrapper child-status retention from the runtime suite result carrier, removal of `cached` from the runtime suite result carrier itself, removal of `root` from the runtime suite result carrier itself, removal of redundant outward suite payload counts in favor of violation-derived reporting, removal of cache-artifact reuse of the public suite payload shape in favor of a minimal violations-plus-cache-metadata artifact, removal of redundant outward suite payload decisions in favor of `PolicySuiteResult.decision()`, removal of outward suite payload timestamps and the hotspot queue's copied source timestamp field, removal of the runtime carrier's `to_payload()` serializer in favor of explicit boundary payload shaping, removal of the redundant runtime `violations_for_rule(...)` accessor in favor of direct carrier reads, removal of the redundant runtime `total_violations()` accessor and wrapper-local hotspot payload helper in favor of direct derivation from canonical violations, replacement of raw child-policy result mappings at the runtime load/scan boundary with a direct semantic boundary value, removal of wrapper child-status traffic from the runtime boundary entirely, removal of the wrapper's temporary child-result mapping rendezvous, removal of raw child-result mapping traffic from wrapper orchestration after ingress validation, removal of the raw child-result parser from the runtime module itself, removal of wrapper-local raw child-payload peelers in favor of a single typed child-artifact loader, removal of the wrapper's remaining `policy_check.py` invocation path in favor of preserve-only child-owned fast-path artifacts, narrowing of the wrapper's remaining child-artifact dependency to only `policy_check_result.json`, final retirement of the wrapper's leftover generic child-artifact loader in favor of a direct policy-check payload loader, removal of obsolete wrapper console status for the projection-semantic-fragment queue, ratcheting of workflow/local-repro orchestration to materialize only `policy_check_result.json` for the wrapper path, removal of cache hash projection from the public suite result/payload surface, removal of the wrapper's dependency on the runtime cache/load outcome in favor of direct scan-plus-boundary artifact writing, final retirement of the runtime cache/load API itself, retirement of the wrapper-owned `policy_suite_results.json` compatibility artifact in favor of direct hotspot-queue publication under an explicit output-directory contract, removal of the runtime scanner's implicit empty child-input fallback in favor of explicit semantic arguments at all call sites, removal of the wrapper's remaining child-status transport surface after child-artifact validation, final retirement of the single-field `PolicySuiteChildInputs` carrier, removal of the wrapper-owned closed rule-family list for console rendering, final removal of pass-through runtime `projection_fiber_semantics` from the suite scanner surface, final removal of the wrapper's whole-payload policy-check helper plus queue-preservation test contract in favor of direct semantic loading and hotspot-only wrapper coverage, transfer of the final minimal hotspot payload assembly step from the suite wrapper to the queue helper, final removal of the queue/report `ProjectionFiberSemanticsSummary` bridge in favor of direct canonical helper projections, transfer of the last `policy_check_result.json` semantic peel from the suite wrapper into the hotspot queue boundary helper, final retirement of the `PolicySuiteResult` compatibility carrier in favor of direct canonical violation maps plus an explicit decision helper, final removal of the runtime module's wrapper-only suite-decision helper so the scan surface stays scan-only, and movement of hotspot-helper coverage out of suite-wrapper tests and into hotspot-owned tests."
...
- `projection_exec.py` remains the compatibility runtime for legacy row-shaped
  `ProjectionSpec` execution, is now explicitly marked as a temporary
  `semantic_carrier_adapter` boundary, and erases semantic-only ops/metadata at
  execution ingress rather than tolerating them by dispatch fallthrough
...
| `2026-03-12` | Legacy projection execution now erases semantic-only ops and semantic metadata at ingress, so `projection_exec.py` no longer depends on silent fallthrough to preserve compatibility for semantic-enriched authoring specs; the file is also explicitly marked as a temporary `semantic_carrier_adapter` boundary to keep that compatibility debt boundary-scoped. | `src/gabion/analysis/projection/projection_exec.py`; `tests/gabion/analysis/projection/test_projection_exec_edges.py`; `tests/gabion/analysis/projection/test_projection_semantic_lowering.py`; `docs/projection_semantic_fragment_rfc.md#projection_semantic_fragment_rfc` |
