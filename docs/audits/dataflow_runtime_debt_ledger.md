...
| `DFD-025` | indexed role-separation seam extraction (`report_sections`, `deadline_fallback`, `statement_materialization`) from `dataflow_indexed_file_scan.py` | strict coverage checkpoint (`97.04% total`) + decomposition mandate | yes | `CU-02` | in_progress | `src/gabion/analysis/indexed_scan/report_sections.py`; `src/gabion/analysis/indexed_scan/deadline_fallback.py`; `src/gabion/analysis/indexed_scan/statement_materialization.py`; `src/gabion/analysis/dataflow_indexed_file_scan.py` | codex | 2026-03-13 | preserve boundary-only ownership in indexed file and import extracted owners directly from downstream helpers |
