...
- WS-5 continuation (`in-349`, this CU):
  - Completed parse-failure witness carrier normalization for remaining indexed-scan flow/materialization seams:
    - `src/gabion/analysis/indexed_scan/scanners/flow/type_flow.py`
      - `infer_type_flow(..., parse_failure_witnesses)` now requires `list[JSONObject]`
    - `src/gabion/analysis/indexed_scan/scanners/flow/constant_flow_details.py`
      - `collect_constant_flow_details(..., parse_failure_witnesses)` now requires `list[JSONObject]`
    - `src/gabion/analysis/indexed_scan/state/module_artifacts.py`
      - `build_module_artifacts(..., parse_failure_witnesses)` now requires `list[JSONObject]`
  - Result:
    - indexed-scan parse-failure witness carrier contracts are now consistently typed to canonical `JSONObject` lists.
  - Validation:
    - policy checks passed
    - private-symbol import guard passed (`new=0`)
    - targeted type-flow/constant-flow/pipeline/obligations suites passed
    - evidence refresh/check passed (`out/test_evidence.json` no drift)
...
- WS-5 continuation (`in-346`, this CU):
  - Removed `None` fallback inside indexed-scan dataclass-registry collector seam:
    - `src/gabion/analysis/indexed_scan/scanners/materialization/dataclass_registry.py`
      - `collect_dataclass_registry(..., stage_cache_fn=...)` now requires an explicit callable argument
      - dropped internal `if stage_cache_fn is None` fallback branch
    - post-phase owner already supplies default callable at boundary (`_analysis_index_stage_cache`), preserving behavior
  - Validation:
    - policy checks passed
    - private-symbol import guard passed (`new=0`)
...
- WS-5 continuation (`in-343`, this CU):
  - Removed remaining private compatibility anchors from facade:
    - `src/gabion/analysis/dataflow/engine/dataflow_facade.py`
      - removed `_build_analysis_index`, `_resolve_callee`, and `_report_section_spec` imports
      - facade now exports public compatibility symbols only
  - Updated compatibility checks to follow private-consumer threshold policy:
    - `tests/gabion/analysis/misc_s3/test_legacy_dataflow_compat_alias_parity.py`
      - selected private facade alias parity checks now execute only when `>1` non-owner, non-test consumers exist
    - `tests/gabion/analysis/misc_s3/test_legacy_dataflow_compat_imports.py`
      - facade smoke assertion now checks public `render_report` instead of private `_report_section_spec`
  - Ratcheted facade metrics guard budgets:
    - `tests/gabion/analysis/misc_s3/test_legacy_dataflow_facade_metrics_guard.py`
      - `_MAX_FACADE_LOC: 71 -> 64`
      - `_MAX_FACADE_TOP_LEVEL_IMPORTS: 17 -> 15`
      - `_MAX_FACADE_IMPORTED_SYMBOLS: 56 -> 53`
      - wildcard cap remains `0`
  - Resulting facade metrics:
    - wildcard imports remain `0`
    - imported symbols `56 -> 53`
    - LOC `71 -> 64`
    - top-level import statements `17 -> 15`
  - Validation:
    - policy checks passed
    - private-symbol import guard passed (`new=0`)
